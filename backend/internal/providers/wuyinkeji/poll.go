...
func (p *Provider) pollResult(ctx context.Context, id string) ([]string, error) {
	...
	for time.Now().Before(deadline) {
		...
		logging.DownstreamRequestDebugRaw("provider_wuyin_detail", p.ProviderName(), http.MethodGet, u, map[string]any{
			"job_id": id,
		}, "", nil)
		...
		if err != nil {
			logging.DownstreamResponseDebugRaw("provider_wuyin_detail_response", p.ProviderName(), http.MethodGet, u, 0, time.Since(start), err, "", nil)
			return nil, err
		}
		b, _ := ioReadAllLimit(resp.Body, 10<<20)
		resp.Body.Close()
		logging.DownstreamResponseDebugRaw("provider_wuyin_detail_response", p.ProviderName(), http.MethodGet, u, resp.StatusCode, time.Since(start), nil, resp.Header.Get("Content-Type"), b)
