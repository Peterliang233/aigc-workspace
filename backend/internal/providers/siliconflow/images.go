...
func (p *Provider) GenerateImage(ctx context.Context, req types.ImageGenerateRequest) (types.ImageGenerateResponse, error) {
	...
	logging.DownstreamRequestRaw("provider_siliconflow_request", p.ProviderName(), http.MethodPost, u, map[string]any{
		"model":               model,
	...
	if err != nil {
		logging.DownstreamResponseRaw("provider_siliconflow_response", p.ProviderName(), http.MethodPost, u, 0, time.Since(start), err, "", nil)
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		logging.DownstreamResponseRaw("provider_siliconflow_response", p.ProviderName(), http.MethodPost, u, resp.StatusCode, time.Since(start), errors.New("bad status"), resp.Header.Get("Content-Type"), b)
	...
	logging.DownstreamResponseRaw("provider_siliconflow_response", p.ProviderName(), http.MethodPost, u, resp.StatusCode, time.Since(start), nil, resp.Header.Get("Content-Type"), b)
